...
an exception is raised if the attribute is accessed::
    ...
    book = session.scalars(select(Book).options(defer(Book.summary, raiseload=True))).first()
...
:func:`.undefer` is required in order for the attribute to be usable::
    ...
    book_w_excerpt = session.scalars(select(Book).options(undefer(Book.excerpt))).first()
