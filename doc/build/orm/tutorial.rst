...
is the :func:`_orm.subqueryload` option, which emits a second SELECT statement
...
constructed directly via the :class:`_query.Query` is re-used, embedded as a subquery
...
.. note::
   ...
   :meth:`_query.Query.first`, :meth:`_query.Query.limit` or :meth:`_query.Query.offset`
   should also include :meth:`_query.Query.order_by` on a unique column in order to
...
While :func:`_orm.joinedload` has been around for a long time, :func:`.subqueryload`
