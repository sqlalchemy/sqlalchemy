...
:class:`_engine.Engine` manages many individual DBAPI connections on behalf of the
process and is intended to be called upon in a concurrent fashion. The
:class:`_engine.Engine` is **not** synonymous to the DBAPI ``connect`` function,
which represents just one connection resource - the :class:`_engine.Engine` is most
...
separate :class:`_engine.Engine` be used for each child process. This is because the
:class:`_engine.Engine` maintains a reference to a connection pool that ultimately
references DBAPI connections - these tend to not be portable across process
boundaries. An :class:`_engine.Engine` that is configured not to use pooling (which
...
:meth:`_engine.Engine.connect` method::
...
The connection is an instance of :class:`_engine.Connection`,
...
:class:`_engine.Connection` is created.
...
When the :meth:`_engine.Connection.close` method is called, the referenced DBAPI
...
:meth:`_engine.Engine.execute` method of :class:`_engine.Engine` itself::

    result = engine.execute("select username from users")
    for row in result:
        print("username:", row['username'])

Where above, the :meth:`_engine.Engine.execute` method acquires a new
:class:`_engine.Connection` on its own, executes the statement with that object,
...
that when its underlying DBAPI cursor is closed, the :class:`_engine.Connection`
...
managers, either given an :class:`_engine.Engine`::
...
Or from the :class:`_engine.Connection`, in which case the :class:`.Transaction` object
...
two functions both issue a transaction on a :class:`_engine.Connection`, but only the outermost
...
:class:`_engine.Engine`, :class:`.Executable`, using the "autocommit" flag which will
