...
def _remove_generated_cython_files(session: nox.Session) -> None:
    """remove .c files generated by Cython from a previous build.

    These are ignored by git, so they remain in a reused checkout.  Cython
    regenerates a .c file only when its .pyx is newer, without regard to
    compiler directives, so a stale .c file would be compiled as is,
    omitting directives set in setup.py such as freethreading_compatible.

    No .c files under lib/ are tracked by git; all are generated.

    """
    for c_file in Path("lib").glob("**/*.c"):
        session.log(f"removing generated file {c_file}")
        c_file.unlink()
...
def _tests(
    ...
    if freethreaded:
        ...
        # greenlet should be fully functional as of py3.14.6t.  fails
        # on py3.14.3t.  but we assume newer python now.
        # greenlet = False
    ...
    _remove_generated_cython_files(session)
