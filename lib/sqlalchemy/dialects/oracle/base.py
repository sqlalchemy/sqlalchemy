...
A dense vector contains meaningful values in most or all of its dimensions.
In contrast, a sparse vector has non-zero values in only a few dimensions,
with the majority being zero.
...
the storage format, and the vector format for the data. Valid values for the
storage format are enum members of :class:`.VectorStorageFormat`. Valid values
for the vector format are enum members of :class:`.VectorFormat`. If vector_format
is not specified, a DENSE vector is created by default.
...
* To create a SPARSE vector with both dimensions and the storage format as flexible::
    ...
    vector_col: Mapped[array.array] = mapped_column(
        VECTOR(vector_format=VectorFormat.SPARSE)
    )
...
Data can be inserted into a sparse vector using a tuple consisting of the number of
dimensions, an array of indices, and a corresponding array of values::
...
class OracleTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_VECTOR(self, type_, **kw):
        ...
        vector_type = (
            type_.vector_format.value
            if type_.vector_format is not None
            else "*"
        )
        return f"VECTOR({dim},{storage_type},{vector_type})"
