    ...
    def _install_unmapped_dataclass_init_warning(self) -> None:
        ...
        def _warn_on_init(self_: Any, *args: Any, **kwargs: Any) -> None:
            if type(self_) is cls_:
                util.warn(
                    f"Direct instantiation of unmapped dataclass "
                    f"{cls_.__name__!r} is not supported and will become an "
                    f"error in a future release."
                )
            orig_init(self_, *args, **kwargs)

        setattr(cls_, "__init__", _warn_on_init)
