...
def eq_regex(a, b, msg=None):
    assert re.match(b, a), msg or "{!r} !~ {!r}".format(a, b)
...
def is_not(a, b, msg=None):
    ...
    assert a is not b, msg or "{!r} is {!r}".format(a, b)
...
def in_(a, b, msg=None):
    ...
    assert a in b, msg or "{!r} not in {!r}".format(a, b)


def not_in(a, b, msg=None):
    """Assert a in not b, with repr messaging on failure."""
    assert a not in b, msg or "{!r} is in {!r}".format(a, b)
...
class AssertsCompiledSQL:
    def assert_compile(
        ...
        eq_(cc, result, "{!r} != {!r} on dialect {!r}".format(cc, result, dialect))
            ...
            for v in values:
                try:
                    if v in pos_by_value:
                        ...
                        start = 0
                    esc = escaped.get(v, v)
                    pos = stmt.index("%({})s".format(esc), start) + 2
                    positions.append(pos)
                    pos_by_value[v].append(pos)
                except ValueError:
                    msg = "Expected to find bindparam {!r} in {!r}".format(v, stmt)
                    assert False, msg
